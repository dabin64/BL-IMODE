...
%     does TP3 has better results?
	ulBestKnownFunctionValue = [225, 0, -18.6787, -29.2, -3.6, -1.20988, -1.96146, 0, 0, 0];
...
if strcmp(benchmark,'SMDmp')
    ACC_ER = 1e-2;
	fn = sprintf('smd%dmp%',fno);
...
% 			s = ceil((llDim - r)/2 + eps);
	...
	[~,~,ulOpt,llOpt]=getOptimalSolutionSMD(ulDim,llDim,sprintf('smd%dmp',fno));
    %% boundaries
       SQP_FEs = 100;
	switch fno
		case 1 %ok
